...
public class BirthDeathSkylineModel extends SpeciesTreeDistribution {
    ...
    public Input<RealParameter> baseReproductiveNumberInput =
            new Input<RealParameter>("baseReproductiveNumber", "The basic / effective reproduction number for the base cluster / pathogen class");
    public Input<RealParameter> lambda_ratioInput =
...
//            if(intervalTimes.getValue(dim-1)==maxTime) changeTimes.add(0.); //rhoSampling
    ...
    */
    ...
    protected void transformParameters() {
        ...
        Double[] R = transform_lambda_base ?  baseReproductiveNumberInput.get().getValues() : reproductiveNumberInput.get().getValues(); // if SAModel: reproductiveNumber = lambda/delta
        Double birth_ratio = transform_lambda_base ? lambda_ratioInput.get().getValue() : 1;
